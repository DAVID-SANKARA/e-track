...
const BudgetItem: React.FC<BudgetItemProps> = ({ budget, enableHover }) => {
  ...
  return (<li key={budget.id} className={`p-4 rounded-xl border-2 border-base-300 list-none ${hoverClasse}`}>
    <div className="flex items-center justify-between">
        ...
        <div className="text-xl font-bold text-accent">{new Intl.NumberFormat('en-US').format(budget.amount)}F CFA</div>
    </div>
    <div className="flex justify-between items-center mt-4 text-gray-500 text-sm">
    <span>{new Intl.NumberFormat('en-US').format(totalTransactionAmount)} F CFA Dépensés</span>
        <span>{new Intl.NumberFormat('en-US').format(remainingAmount)}F CFA Restant</span>
    </div>
    <div>
